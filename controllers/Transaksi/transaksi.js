  ...
  try {
    ...
    for (const item of produk) {
      ...
      if (!produkData) {
        return res
          .status(404)
          .json({ message: `Produk dengan id ${idProduk} tidak ditemukan` });
      }
      if (produkData.stokProduk < jumlah) {
        return res.status(400).json({
          message: `Stok produk dengan id ${idProduk} tidak mencukupi`,
        });
      }
      ...
      include: [
        {
          ...
          include: [
            { model: Produk, attributes: ["id", "namaProduk", "hargaProduk"] },
          ],
        },
